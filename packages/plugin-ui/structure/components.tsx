...
import { getStructureThemeTypes } from 'molstar/lib/mol-plugin-state/helpers/structure-representation-params'
...
export class StructureComponentControls extends CollapsableControls<
  {},
  StructureComponentControlState
> {
...
> {
  ...
  componentDidMount() {
    this.subscribe(
      ...
      (c) =>
        this.setState({
          action:
            this.state.action !== 'options' || c.structures.length === 0
              ? void 0
              : 'options',
  ...
  get presetActions() {
    ...
    const providers = this.plugin.builders.structure.representation.getPresets(
      pivot?.cell.obj
    )
...
export class AddComponentControls extends PurePluginUIComponent<
  AddComponentControlsProps,
  AddComponentControlsState
> {
...
> {
  ...
  get colorByActions() {
    ...
    const themes = getStructureThemeTypes(
      this.plugin,
      this.pivot.cell.obj?.data
    )
    return ActionMenu.createItemsFromSelectOptions(themes, {
      value: (o) => () =>
        mng.updateRepresentationsTheme(this.props.group, {
          color: o[0] as any,
        }),
      selected: (o) => o[0] === name,
    }) as ActionMenu.Item[]
    ...
    const ret: ActionMenu.Items = [
      [
        ...
        ...StructureComponentManager.getRepresentationTypes(
          ...
          ActionMenu.Item(t[1], () =>
            mng.addRepresentation(this.props.group, t[0])
          )
        ),
      ],
    ]
  ...
  remove = () =>
    this.plugin.managers.structure.hierarchy.remove(this.props.group, true)

  toggleAction = () =>
    this.setState({
      action: this.state.action === 'action' ? void 0 : 'action',
    })
  toggleLabel = () =>
    this.setState({ action: this.state.action === 'label' ? void 0 : 'label' })
  ...
  get reprLabel() {
    ...
    if (pivot.representations.length === 1)
      return pivot.representations[0].cell.obj?.label
  ...
  render() {
    ...
    return (
      <div className='msp-representation-entry'>
        {repr.parent && (
          <ExpandGroup
            header={`${repr.obj?.label || ''} Representation`}
            noOffset
          >
        ...
        <IconButton
          svg={
            this.props.representation.cell.state.isHidden
              ? VisibilityOffOutlinedSvg
              : VisibilityOutlinedSvg
          }
          toggleState={false}
