...
export class StructureSelectionStatsControls extends PluginUIComponent<
  { hideOnEmpty?: boolean },
  { isEmpty: boolean; isBusy: boolean }
> {
  state = {
    isEmpty: true,
    isBusy: false,
...
const ResidueListIdTypeParams = {
  idType: ParamDefinition.Select<'auth' | 'label' | 'atom-id'>(
    'auth',
    ParamDefinition.arrayToOptions(['auth', 'label', 'atom-id'])
  ),
  identifiers: ParamDefinition.Text('', {
    description:
      'A comma separated list of atom identifiers (e.g. 10, 15-25) or residue ranges in given chain (e.g. A 10-15, B 25, C 30:i)',
  }),
}
