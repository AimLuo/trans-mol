...
import { SIFTSMapping } from 'molstar/lib/mol-model-props/sequence/sifts-mapping'
...
const DefaultStructureSuperpositionOptions = PD.getDefaultValues(
  StructureSuperpositionParams
)
export type StructureSuperpositionOptions = PD.ValuesFor<
  typeof StructureSuperpositionParams
>
...
> {
      ...
      (sel) => {
        this.setState({
          canUseDb: sel.structures.every(
            (s) =>
              !!s.cell.obj?.data &&
              s.cell.obj.data.models.some((m) =>
                SIFTSMapping.Provider.isApplicable(m)
              )
          ),
        })
      }
  ...
  async transform(
    ...
    const o = StateSelection.findTagInSubtree(
  ...
  superposeChains = async () => {
    ...
    const locis = entries.map((e, i) => {
      ...
      const loci = StructureSelection.toLociWithSourceUnits(
        query(new QueryContext(s))
      )
      return StructureElement.Loci.remap(
        loci,
        i === 0
          ? this.plugin.helpers.substructureParent.get(e.loci.structure.root)!
              .obj!.data
          : loci.structure.root
      )
    })
  ...
  superposeDb = async () => {
    const input =
      this.plugin.managers.structure.hierarchy.behaviors.selection.value
        .structures
    const traceOnly = this.state.options.traceOnly

    const structures = input.map((s) => s.cell.obj?.data!)
    const { entries, failedPairs, zeroOverlapPairs } =
      alignAndSuperposeWithSIFTSMapping(structures, { traceOnly })
    ...
    if (zeroOverlapPairs.length) {
      this.plugin.log.warn(
        `Superposition: No UNIPROT mapping overlap between structures ${formatPairs(
          zeroOverlapPairs
        )}.`
      )
    }
    ...
    if (failedPairs.length) {
      this.plugin.log.error(
        `Superposition: Failed to superpose structures ${formatPairs(
          failedPairs
        )}.`
      )
    }
    ...
    if (entries.length) {
      this.plugin.log.info(
        `Superposed ${
          entries.length + 1
        } structures with avg. RMSD ${rmsd.toFixed(2)} Å.`
      )
      await this.cameraReset()
    }
  }
  ...
  toggleByChains = () =>
    this.setState({
      action: this.state.action === 'byChains' ? void 0 : 'byChains',
    })
  toggleByAtoms = () =>
    this.setState({
      action: this.state.action === 'byAtoms' ? void 0 : 'byAtoms',
    })
  toggleOptions = () =>
    this.setState({
  ...
  moveHistory(e: StructureSelectionHistoryEntry, direction: 'up' | 'down') {
    this.plugin.managers.structure.selection.modifyHistory(
      e,
      direction,
      void 0,
      true
  ...
  get chainEntries() {
    ...
    this.plugin.managers.structure.selection.entries.forEach(
      ({ selection }, ref) => {
        ...
        {entries.length < 2 && (
          <div className='msp-control-offset msp-help-text'>
            <div className='msp-help-description'>
              ...
              Add 2 or more selections (toggle{' '}
              <ToggleSelectionModeButton inline /> mode) from separate
              structures. Selections must be limited to single polymer chains or
              residues therein.
            </div>
          </div>
        )}
      ...
      <>
        ...
        {entries.length < 2 && (
          <div className='msp-control-offset msp-help-text'>
            <div className='msp-help-description'>
              <Icon svg={HelpOutlineSvg} inline />
              Add 1 or more selections (toggle{' '}
              <ToggleSelectionModeButton inline /> mode) from separate
              structures. Selections must be limited to single atoms.
            </div>
          </div>
        )}
        {entries.length > 1 && (
