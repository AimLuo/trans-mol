...
> {
  ...
  getTrajectoryItems = (t: TrajectoryRef): ActionMenu.Items => {
    ...
    return [
      ActionMenu.Header(t.cell.obj?.label!),
      ...t.models.map(this.getModelItems),
    ]
  }
  ...
  private getModelItems = (m: ModelRef): ActionMenu.Items => {
    ...
    return [
      ActionMenu.Header(m.cell.obj?.label!),
      ...m.structures.map(this.item),
    ]
  }
  ...
  get hierarchyItems() {
    ...
    if (current.models.length > 1 || current.trajectories.length > 1) {
      ret.push([ActionMenu.Header('Models'), ...current.models.map(this.item)])
    }
  ...
  get presetActions() {
    ...
    for (const p of providers) {
      actions.push(
        ActionMenu.Item(p.display.name, p, {
  ...
  get modelIndex() {
    ...
    if (
      !m ||
      m.cell.transform.transformer !== StateTransforms.Model.ModelFromTrajectory
    )
      return null
