...
import { VolumeHierarchyManager } from 'molstar/lib/mol-plugin-state/manager/volume/hierarchy'
import {
  LazyVolumeRef,
  VolumeRef,
  VolumeRepresentationRef,
} from 'molstar/lib/mol-plugin-state/manager/volume/hierarchy-state'
...
> {
  ...
  renderEnable() {
    ...
    const simpleApply =
      rootCell && rootCell.status === 'error'
        ? {
            header:
              !!rootCell.errorText && rootCell.errorText?.includes('404')
  ...
  renderParams() {
    ...
    if (!pivot.cell.parent) return null
    const bindings =
      pivot.volumeStreaming?.cell.transform.params?.entry.params.view.name ===
        'selection-box' &&
      this.plugin.state.behaviors.cells.get(FocusLoci.id)?.params?.values
        ?.bindings
    return (
      <>
...
> {
  ...
  componentDidMount() {
    this.subscribe(
      this.plugin.managers.volume.hierarchy.behaviors.selection,
      (sel) => {
        this.setState({
          isHidden:
            sel.hierarchy.volumes.length === 0 &&
  ...
  get addActions(): ActionMenu.Items {
    ...
    const ret: ActionMenu.Items = [
      ...VolumeHierarchyManager.getRepresentationTypes(
        this.plugin,
        current
      ).map((t) =>
        ActionMenu.Item(t[1], () => mng.addRepresentation(current!, t[0]))
      ),
    ]
  ...
  get isEmpty() {
    const { volumes, lazyVolumes } =
      this.plugin.managers.volume.hierarchy.current
  ...
  toggleHierarchy = () =>
    this.setState({
      show: this.state.show !== 'hierarchy' ? 'hierarchy' : void 0,
    })
  toggleAddRepr = () =>
    this.setState({
      show: this.state.show !== 'add-repr' ? 'add-repr' : void 0,
    })
...
> {
  ...
  remove = () =>
    this.plugin.managers.volume.hierarchy.remove(
  ...
  toggleUpdate = () =>
    this.setState({
      action: this.state.action === 'update' ? void 0 : 'update',
    })
