...
} from 'molstar/lib/mol-plugin-state/manager/structure/measurement'
...
export class MeasurementList extends PurePluginUIComponent {
  componentDidMount() {
    this.subscribe(
      this.plugin.managers.structure.measurement.behaviors.state,
      () => {
        this.forceUpdate()
      }
    )
  ...
  renderGroup(cells: ReadonlyArray<StructureMeasurementCell>, header: string) {
    ...
    for (const cell of cells) {
      if (cell.obj)
        group.push(<MeasurementEntry key={cell.obj.id} cell={cell} />)
    }
    return group.length ? (
      <ExpandGroup header={header} initiallyExpanded={true}>
        {group}
      </ExpandGroup>
    ) : null
  }
...
export class MeasurementControls extends PurePluginUIComponent<
  {},
  { isBusy: boolean; action?: 'add' | 'options' }
> {
  ...
  componentDidUpdate(
    prevProps: {},
    prevState: { isBusy: boolean; action?: 'add' | 'options' }
  ) {
    if (this.state.action !== prevState.action) this.updateOrderLabels()
  }
  ...
  measureDistance = () => {
    ...
    this.plugin.managers.structure.measurement.addDistance(
      loci[0].loci,
      loci[1].loci
    )
  }
  ...
  toggleAdd = () =>
    this.setState({ action: this.state.action === 'add' ? void 0 : 'add' })
  toggleOptions = () =>
    this.setState({
  ...
  add() {
    ...
    return (
      <>
        ...
        {entries.length > 0 && (
          <div className='msp-control-offset'>{entries}</div>
        )}
        {entries.length === 0 && (
          <div className='msp-control-offset msp-help-text'>
            <div className='msp-help-description'>
              <Icon svg={HelpOutlineSvg} inline />
              Add one or more selections (toggle{' '}
              <ToggleSelectionModeButton inline /> mode)
            </div>
          </div>
        )}
      </>
...
> {
  ...
  highlight = () => {
    ...
    this.plugin.managers.interactivity.lociHighlights.highlight(
      { loci: this.props.cell.obj?.data.repr.getLoci()! },
      false
    )
  }
  ...
  get label() {
    ...
    if (selections.pairs)
      return distanceLabel(selections.pairs[0], {
        condensed: true,
        unitLabel:
          this.plugin.managers.structure.measurement.state.options
            .distanceUnitLabel,
      })
