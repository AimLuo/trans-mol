...
function addSymmetryGroupEntries(
  ...
  const idx = SortedArray.indexOf(
    location.unit.elements,
    location.element
  ) as UnitIndex
  ...
  for (const ug of structure.unitSymmetryGroups) {
    ...
    if (isBirdMolecule) {
      ...
      const { index: residueIndex } =
        u.model.atomicHierarchy.residueAtomSegments
      let prev = -1
...
> {
  ...
  componentDidMount() {
    ...
    this.subscribe(
      this.plugin.managers.structure.focus.events.historyUpdated,
      (c) => {
        this.forceUpdate()
      }
    )
  ...
  getSelectionItems = memoizeLatest(
  ...
  getToggleBindingLabel() {
    const t = this.plugin.state.behaviors.transforms.get(
      FocusLoci.id
    ) as StateTransform<typeof FocusLoci>
    if (!t) return ''
