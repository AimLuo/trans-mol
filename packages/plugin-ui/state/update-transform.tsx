...
import { State, StateTransform, StateTransformer } from 'molstar/lib/mol-state'
...
namespace UpdateTransformControl {
  ...
  export type ComponentState = TransformControlBase.ComponentState
}
...
> {
  ...
  getHeader() {
    return (
      ...
      this.props.transform.transformer.definition.display
    )
  }
  canApply() {
  ...
  componentDidMount() {
    ...
    if (this.props.toggleCollapsed)
      this.subscribe(this.props.toggleCollapsed, () =>
      ...
      ({ ref, state }) => {
        ...
        if (this.state.params !== this.props.transform.params) {
          this._getInfo = memoizeLatest((t: StateTransform) =>
            StateTransformParameters.infoFromTransform(
              this.plugin,
              this.props.state,
              t
            )
          ...
          this.setState({
            params: this.props.transform.params,
            isInitial: true,
          })
        }
      }
