    ...
    if (this.state.showActions) {
      return (
        <div style={{ margin: '10px', cursor: 'default' }}>
          ...
          <p>
            Structures and Volumes can be loaded from the{' '}
            <Icon svg={HomeOutlinedSvg} /> tab.
          </p>
        </div>
      )
    }
...
> {
  ...
  componentDidMount() {
    this.subscribe(this.plugin.state.events.cell.stateUpdated, (e) => {
      if (
        this.props.cell === e.cell &&
        this.is(e) &&
        e.state.cells.has(this.ref)
      ) {
        if (
    ...
    this.subscribe(this.plugin.state.events.cell.created, (e) => {
      if (
        this.props.cell.parent === e.state &&
        this.ref === e.cell.transform.parent
      ) {
        this.forceUpdate()
      }
    })
  ...
  static getDerivedStateFromProps(
    ...
    if (
      !!props.cell.state.isCollapsed === state.isCollapsed &&
      state.isNull === isNull &&
  ...
  private static hasDecorator(cell: StateObjectCell) {
    ...
    return !!cell.parent?.tree.transforms.get(children.first()).transformer
      .definition.isDecorator
  }
  ...
  private static showLabel(cell: StateObjectCell) {
    return (
      cell.transform.ref !== StateTransform.RootRef &&
      (cell.status !== 'ok' ||
        (!cell.state.isGhost && !StateTreeNode.hasDecorator(cell)))
    )
  }
...
> {
  ...
  get actions() {
    ...
    return [
      ...
      ...actions.map((a) =>
        ActionMenu.Item(a.definition.display.name, () =>
  ...
  updates(margin: string) {
    const cell = this.props.cell
    const decoratorChain = StateTreeSpine.getDecoratorChain(
      cell.parent!,
      cell.transform.ref
    )
  ...
  render() {
    ...
    if (cell.status === 'error' || !cell.obj) {
      ...
      label = (
        <Button
          ...
          disabled={disabled}
        >
          {cell.status === 'error' && <b>[{cell.status}]</b>}{' '}
          <span>{name}</span>
        </Button>
      )
    } else {
    ...
    const visibility = (
      <IconButton
        svg={
          cellState.isHidden ? VisibilityOffOutlinedSvg : VisibilityOutlinedSvg
        }
        toggleState={false}
        disabled={disabled}
        small
    ...
    const row = (
      <div
        className={`msp-flex-row msp-tree-row${
          isCurrent ? ' msp-tree-row-current' : ''
        }`}
