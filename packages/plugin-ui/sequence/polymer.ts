...
import { Interval, OrderedSet, SortedArray } from 'molstar/lib/mol-data/int'
...
import { MissingResidues } from 'molstar/lib/mol-model/structure/model/properties/common'
    ...
    if (StructureElement.Loci.is(loci)) {
      ...
      for (const e of loci.elements) {
        ...
        if (Unit.isAtomic(e.unit)) {
          changed =
            applyMarkerAtomic(e, action, this.markerArray, index) || changed
        } else {
          changed =
            applyMarkerCoarse(e, action, this.markerArray, index) || changed
        }
      }
...
function applyMarkerCoarse(
  ...
  OrderedSet.forEach(e.indices, (i) => {
    const eI = elements[i]
    for (
      let s = index(begin.value(eI)), e = index(end.value(eI));
      s <= e;
      s++
    ) {
