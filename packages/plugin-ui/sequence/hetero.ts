...
export class HeteroSequenceWrapper extends SequenceWrapper<StructureUnit> {
  ...
  mark(loci: Loci, action: MarkerAction) {
    ...
    if (StructureElement.Loci.is(loci)) {
      ...
      for (const e of loci.elements) {
        ...
        if (unit) {
          ...
          OrderedSet.forEach(e.indices, (v) => {
            const seqIdx = this.sequenceIndices.get(
              residueIndex[unit.elements[v]]
            )
            if (
              seqIdx !== undefined &&
              applyMarkerAction(
                this.markerArray,
                Interval.ofSingleton(seqIdx),
                action
              )
            )
              changed = true
      ...
      if (
        applyMarkerAction(
          this.markerArray,
          Interval.ofBounds(0, this.length),
          action
        )
      )
        changed = true
  ...
  getLoci(seqIdx: number) {
    ...
    if (rI !== undefined) {
      ...
      const end = SortedArray.findPredecessorIndex(
        unit.elements,
        offsets[rI + 1]
      )
  ...
  constructor(data: StructureUnit) {
    ...
    for (let i = 0, il = data.units.length; i < il; ++i) {
      ...
      while (residueIt.hasNext) {
        ...
        sequence.push(
          atoms.label_comp_id.value(residueAtomSegments.offsets[index])
        )
      }
    }
