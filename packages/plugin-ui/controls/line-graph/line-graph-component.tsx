...
> {
    ...
    return [
      <div key='LineGraph'>
        <svg
          ...
          viewBox={`0 0 ${this.width + this.padding} ${
        ...
        >
          ...
          <g className='ghost-points' stroke='black' fill='black'></g>
        </svg>
      </div>,
  ...
  componentDidMount() {
    this.gElement = document.getElementsByClassName(
      'ghost-points'
    )[0] as SVGElement
  }
  ...
  private handleMouseDown = (id: number) => (event: any) => {
    ...
    this.ghostPoints.push(
      document.createElementNS(this.namespace, 'circle') as SVGElement
    )
  ...
  private handlePointUpdate(event: any) {
    ...
    const updatedPoint = this.unNormalizePoint(
      Vec2.create(this.updatedX, this.updatedY)
    )
  ...
  private unNormalizePoint(point: Vec2) {
    ...
    const unNormalizedY =
      (this.height + this.padding - point[1] - min) / (maxY - min)
