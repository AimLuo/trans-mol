...
import { Mat4, Vec2, Vec3 } from 'molstar/lib/mol-math/linear-algebra'
...
} from 'molstar/lib/mol-util/color/lists'
import { Legend as LegendData } from 'molstar/lib/mol-util/legend'
import { memoize1, memoizeLatest } from 'molstar/lib/mol-util/memoize'
import { getPrecision } from 'molstar/lib/mol-util/number'
import { ParamDefinition as PD } from 'molstar/lib/mol-util/param-definition'
import { ParamMapping } from 'molstar/lib/mol-util/param-mapping'
import { camelCaseToWords } from 'molstar/lib/mol-util/string'
...
export class ParameterControls<P extends PD.Params> extends React.PureComponent<
...
> {
  ...
  renderGroup(group: ParamInfo[]) {
    ...
    if (category) {
      return [
        <ExpandGroup key={category} header={category}>
          {ctrls}
        </ExpandGroup>,
      ]
    }
  ...
  render() {
    ...
    if (essentials && advanced) {
      return (
        <>
          {essentials}
          <ExpandGroup header='Advanced Options'>{advanced}</ExpandGroup>
        </>
      )
    } else if (essentials) {
      return essentials
...
export class ParameterMappingControl<S, T> extends PluginUIComponent<{
  ...
  setSettings = (
    p: { param: PD.Base<any>; name: string; value: any },
    old: any
  ) => {
...
function classifyParams(params: PD.Params) {
  ...
  const essentials: {
    params: ParamInfo[][]
    map: Map<string, ParamInfo[]> | undefined
  } = { params: [[]], map: void 0 }
...
function controlFor(param: PD.Any): ParamControl | undefined {
  switch (param.type) {
    ...
    case 'boolean':
      return BoolControl
    case 'number':
      return typeof param.min !== 'undefined' &&
        typeof param.max !== 'undefined'
        ? NumberRangeControl
        : NumberInputControl
    case 'converted':
...
export class ParamHelp<L extends LegendData> extends React.PureComponent<{
  legend?: L
  description?: string
}> {
  render() {
...
export abstract class SimpleParam<P extends PD.Any> extends React.PureComponent<
  ParamProps<P>,
  { showHelp: boolean }
> {
...
> {
  ...
  render() {
    const placeholder =
      this.props.param.label || camelCaseToWords(this.props.name)
    const label = this.props.param.label || camelCaseToWords(this.props.name)
    const p = getPrecision(this.props.param.step || 0.01)
    return (
      <ControlRow
  ...
  renderControl() {
    ...
    return (
      <Slider
        ...
        onChange={this.onChange}
        onChangeImmediate={
          this.props.param.immediateUpdate ? this.onChange : void 0
        }
        disabled={this.props.isDisabled}
        onEnter={this.props.onEnter}
      />
      ...
      >
        {isInvalid && (
          <option
            ...
            value={this.props.value}
          >{`[Invalid] ${this.props.value}`}</option>
        )}
...
> {
  ...
  renderAddOn() {
    ...
    return (
      <ActionMenu items={items} current={current} onSelect={this.onSelect} />
    )
  }
...
> {
  ...
  items = memoizeLatest((param: PD.ValueRef) =>
    ActionMenu.createItemsFromSelectOptions(param.getOptions())
  )
...
function colorStripStyle(
  list: PD.ColorList['defaultValue'],
  right = '0'
): React.CSSProperties {
  return {
...
function colorGradient(colors: ColorListEntry[], banded: boolean) {
  return banded
    ? colorGradientBanded(colors)
    : colorGradientInterpolated(colors)
}
...
function createColorListHelpers() {
  ...
  return {
    ...
    ColorsParam: PD.ObjectList({ color: PD.Color(0x0 as Color) }, ({ color }) =>
      Color.toHexString(color).toUpperCase()
    ),
...
> {
  ...
  toggleEdit = () =>
    this.setState({ show: this.state.show === 'edit' ? void 0 : 'edit' })
  togglePresets = () =>
  ...
  colorsChanged: ParamOnChange = ({ value }) => {
    this.update({
      ...
      colors: (
        value as typeof _colorListHelpers['ColorsParam']['defaultValue']
      ).map((c) => c.color),
    })
  }
  ...
  isInterpolatedChanged: ParamOnChange = ({ value }) => {
    this.update({
      ...
      colors: this.props.value.colors,
    })
  }

  renderColors() {
    if (!this.state.show) return null
    const { ColorPresets, ColorsParam, IsInterpolatedParam } =
      ColorListHelpers()
...
> {
  ...
  colorsChanged: ParamOnChange = ({ value }) => {
    const colors = (
      value as typeof _colorListHelpers['OffsetColorsParam']['defaultValue']
  ...
  renderColors() {
    if (!this.state.show) return null
    const { ColorPresets, OffsetColorsParam, IsInterpolatedParam } =
      ColorListHelpers()
...
> {
  ...
  components = {
    0: PD.Numeric(
      ...
      {
        label:
          (this.props.param.fieldLabels && this.props.param.fieldLabels.x) ||
          'X',
      }
    ),
    1: PD.Numeric(
      ...
      {
        label:
          (this.props.param.fieldLabels && this.props.param.fieldLabels.y) ||
          'Y',
      }
    ),
    2: PD.Numeric(
      ...
      {
        label:
          (this.props.param.fieldLabels && this.props.param.fieldLabels.z) ||
          'Z',
      }
    ),
  }
...
> {
  ...
  get grid() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      rows.push(
        <div className='msp-flex-row' key={i}>
          {row}
        </div>
      )
    }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {this.presets()}
        {this.state.isExpanded && (
          <div className='msp-control-offset'>{controls}</div>
        )}
      </div>
    )
  }
...
> {
  ...
  render() {
    const value: PD.Mapped<any>['defaultValue'] =
      this.props.value || this.props.param.defaultValue
...
export class ConditionedControl extends React.PureComponent<
  ParamProps<PD.Conditioned<any, any, any>>
> {
...
export class ConvertedControl extends React.PureComponent<
  ParamProps<PD.Converted<any, any>>
> {
