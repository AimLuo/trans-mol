...
import { ArrowDropDownSvg, ArrowRightSvg, CheckSvg, CloseSvg } from './icons'
...
export namespace ActionMenu {
  ...
  export function createItems<T>(
    xs: ArrayLike<T>,
    params?: CreateItemsParams<T>
  ): Items[] {
    const { label, value, category, selected, icon, addOn, description } =
      params || {}
    let cats: Map<string, (ActionMenu.Item | ActionMenu.Header)[]> | undefined =
      void 0
    ...
    for (let i = 0; i < xs.length; i++) {
      ...
      if (!!catName) {
        if (!cats)
          cats = new Map<string, (ActionMenu.Item | ActionMenu.Header)[]>()
  ...
  export function createItemsFromSelectOptions<
    O extends ParamDefinition.Select<any>['options']
  >(options: O, params?: CreateItemsParams<O[0]>) {
    return createItems(
      options,
      params ? { ..._selectOptions, ...params } : _selectOptions
    )
  }
...
class Section extends React.PureComponent<SectionProps, SectionState> {
  static createState(props: SectionProps, isExpanded?: boolean): SectionState {
    const header =
      isItems(props.items) && isHeader(props.items[0]) ? props.items[0] : void 0
    ...
    const hasCurrent = header?.isIndependent
      ...
      : (!!props.current &&
          !!ActionMenu.findItem(props.items, props.current.value)) ||
        ActionMenu.hasSelectedItem(props.items)
  ...
  componentDidUpdate(prevProps: SectionProps) {
    if (
      ...
      const isExpanded =
        isItems(this.props.items) &&
        isItems(prevProps.items) &&
        isHeader(this.props.items[0]) &&
        isHeader(prevProps.items[0]) &&
  ...
  render() {
    ...
    return (
      <>
        {header &&
          (this.props.multiselect && this.state.isExpanded
...
const Action: React.FC<{
  ...
  const style: React.CSSProperties | undefined = item.addOn
    ? { position: 'relative' }
    : void 0
