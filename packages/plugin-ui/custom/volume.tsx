...
import { StateObjectCell, StateSelection } from 'molstar/lib/mol-state'
import { ColorNames } from 'molstar/lib/mol-util/color/names'
import { toPrecision } from 'molstar/lib/mol-util/number'
    ...
    return (
      <ExpandableControlRow
        ...
        controls={
          <ParameterControls
            onChange={({ name, value }) =>
              props.changeParams(props.name, name, value)
            }
...
export class VolumeStreamingCustomControls extends PluginUIComponent<StateTransformParameters.Props> {
  ...
  changeOption: ParamOnChange = ({ name, value }) => {
    ...
    if (name === 'entry') {
      ...
      const oldView =
        old.entry.params.view.name === value.name
          ...
          : (
              (
                (
                  ...
                  old.entry.name
                ) as PD.Group<VolumeStreaming.EntryParamDefinition>
  ...
  render() {
    ...
    const isRelative =
      ((params.entry.params.channels as any)[pivot].isoValue as Volume.IsoValue)
        .kind === 'relative'
    ...
    const isUnboundedParam = PD.Boolean(isUnbounded, {
      description:
        'Show full/limited range of iso-values for more fine-grained control.',
      label: 'Unbounded',
    })
    ...
    const OptionsParams = {
      entry: PD.Select(
        params.entry.name,
        b.data.entries.map(
          (info) => [info.dataId, info.dataId] as [string, string]
        ),
        {
          isHidden: isOff,
      ...
      view: PD.MappedStatic(
        ...
        },
        {
          ...
          description:
            'Controls what of the volume is displayed. "Off" hides the volume alltogether. "Bounded box" shows the volume inside the given box. "Around Focus" shows the volume around the element/atom last interacted with. "Whole Structure" shows the volume for the whole structure.',
        }
      ),
    }
    const options = {
      ...
      view: {
        ...
        params: {
          ...
          selectionDetailLevel: (params.entry.params.view.params as any)
            .selectionDetailLevel,
          isRelative,
          isUnbounded,
        },
