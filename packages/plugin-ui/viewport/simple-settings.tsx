...
import { Mutable } from 'molstar/lib/mol-util/type-helpers'
...
const SimpleSettingsParams = {
  ...
  layout: PD.MultiSelect(
    [] as LayoutOptions[],
    PD.objectToOptions(LayoutOptions)
  ),
}
...
const SimpleSettingsMapping = ParamMapping({
  ...
  async apply(props, ctx) {
    ...
    const state = produce(ctx.layout.state, (s) => {
      s.regionState.top =
        props.layout.indexOf('sequence') >= 0 ? 'full' : 'hidden'
      s.regionState.bottom =
        props.layout.indexOf('log') >= 0 ? 'full' : 'hidden'
